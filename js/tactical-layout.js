...
(function () {
  ...
  function isDrawerVisible() {
    ...
    if (!d) return false;
    // BẢO VỆ TẬN GỐC: Chưa chọn sự cố thì drawer vĩnh viễn không được coi là visible
    if (!window.dispatcher || !window.dispatcher.selectedIncidentId) return false;
    var disp = d.style.display;
    if (disp === 'none' || d.classList.contains('is-hidden')) return false;
    return true;
  }

  var drawerObserver = null;
  var isSyncingDrawer = false;
  var resizeMapTimer = null;
  ...
  function safeSetDrawerState(visible) {
    var d = els.drawer || $('activeIncidentDrawer');
    if (!d) return;

    var targetDisplay = visible ? 'flex' : 'none';
    var hasClass = d.classList.contains('is-hidden');
    var shouldHaveClass = !visible;

    // Check if DOM already matches desired state
    var displayMatches = (visible && d.style.display !== 'none' && !d.classList.contains('is-hidden')) ||
                         (!visible && (d.style.display === 'none' || d.classList.contains('is-hidden')));

    if (displayMatches && hasClass === shouldHaveClass) {
      return; // No mutation needed! Prevents redundant work
    }

    // Disconnect drawerObserver while modifying attributes to eliminate feedback loop
    if (drawerObserver) {
    ...
    try {
      ...
      // Purge any mutation records generated by our own modification and re-observe
      if (drawerObserver && d) {
        try {
          drawerObserver.takeRecords();
          drawerObserver.observe(d, { attributes: true, attributeFilter: ['style', 'class'] });
        } catch(e) {}
      }
    }
  }
  ...
  function moveDrawer(toPanel) {
    ...
    // Chỉ khi có sự cố được chọn mới di chuyển drawer vào tacticalDetailBody
    if (toPanel) {
  ...
  function setMobileView(view, byUser) {
    ...
    var empty = $('tacticalDetailEmpty');
    var hasSelected = Boolean(window.dispatcher && window.dispatcher.selectedIncidentId);
    document.body.classList.toggle('tac-incident-selected', hasSelected);
  ...
  function observe() {
    if (els.drawer && window.MutationObserver) {
      drawerObserver = new MutationObserver(function () { syncDrawerState(); });
      drawerObserver.observe(els.drawer, { attributes: true, attributeFilter: ['style', 'class'] });
      var sid = $('drawerSosId');
