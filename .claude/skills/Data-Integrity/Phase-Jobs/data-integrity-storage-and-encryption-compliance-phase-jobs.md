...
# Mode 1 Prompt
Use this prompt to run a full storage-and-encryption-compliance data-integrity sweep for the active unfinished phase/job.
...
- any repo-owned SPEC that defines allowed server-side persistence for the active phase/job
...
- If the declared phase/job is broader than this lane, keep the full phase/job and exact SPEC family as context, then narrow only the review work to the storage-and-encryption-compliance surfaces inside that declared scope.
...
2. Resolve the active storage and encryption surface inside the declared scope:
...
3. Build the storage map for the declared surface:
...
Example:
```text
[HIGH] SQLCipher key for active scope data is persisted in OS keyring instead of .dbkey
Broken invariant: SQLCipher keys must use the .dbkey file pattern; OS keyring is for JWT tokens only
Expected: database key is stored at the repo-owned .dbkey path with owner-only permissions
Actual: local DB open path reads and writes the SQLCipher key via keyring, so OS-profile loss can destroy DB access
Files:
- backend/internal/storage/key_provider.go:41
- backend/internal/storage/sqlcipher_open.go:18
Fix: move SQLCipher key persistence to the repo-owned .dbkey path and keep keyring usage limited to JWT tokens.
```
